...
== Options
      ...
      `--id` _string_::         If a unique ID is not set for the Kafka instance you want to view, the current Kafka instance will be utilised
  `-o`, `--output` _string_::   Choose a format from: "json", "yml", "yaml", to display the Kafka instance (default "json")
