					...
					for _, condition := range conditions {
						...
						if typedCondition["type"].(string) == "Finished" {
							...
							if typedCondition["status"].(string) == "True" {
								c.logger.Info(build.EmojiSuccess, c.localizer.MustLocalize("cluster.kubernetes.watchForKafkaStatus.log.info.success", localize.NewEntry("Name", crName), localize.NewEntry("Namespace", namespace)))
