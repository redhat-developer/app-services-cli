...
func NewCreateCommand(f *factory.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !f.IOStreams.CanPrompt() && opts.name == "" {
				return f.Localizer.MustLocalizeError("connector.create.argument.name.error.requiredWhenNonInteractive")
			} else if opts.name == "" {
	...
	flags.StringVar(&opts.name, "name", "", f.Localizer.MustLocalize("connector.name.flag.id.description"))
