...
func TestNewUseCommand(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Successfully sets current context to a valid context",
			args: args{
				context: &servicecontext.Context{
		...
		{
			name: "Should fail if a non-existing context is specified",
			args: args{
				context: &servicecontext.Context{
					Contexts: map[string]servicecontext.ServiceConfig{
						"c1": {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("Setting current context to \"%s\" throws error: %v", tt.targetContext, err)
			}
		})
	}
}
